...
| `Ruzsa` | [Ruzsa (1983), Theorem, p. 579; (2.3)–(2.4), p. 578](https://doi.org/10.1007/978-3-0348-5438-2_50); weighted-center form in Mangerel, Lemma 3.3, p. 1038 | Center `Σ f(p^k)(1-1/p)/p^k`, whereas Ruzsa's original center is `Σ f(p)/p` over primes. Absolute `c, C, X₀`, with `X₀ ≥ 3`; the two-sided estimate is assumed for `X ≥ X₀`. The lower bound is used as `X → ∞`; [FiniteCutoffMoment.lean](Erdos1122/FiniteCutoffMoment.lean) extends the upper bound to smaller scales. |
