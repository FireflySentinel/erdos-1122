...
Theorem~X is stated without proof in \cite[p.~17]{Erdos}; the source
of this final distribution implication is discussed below.
...
A later source for the distribution argument is Hildebrand's
Erd\H{o}s--Wintner theorem for differences \cite{Hildebrand}.
Its limiting distribution is symmetric, so absence of mass on
$(-\infty,0)$ makes it a point mass at zero. Hildebrand's density-one
characterization of the logarithm then gives the required conclusion.
\end{remark}
...
formalization checks the implication from the five cited results, which
