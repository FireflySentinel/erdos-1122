...
The implication from the four cited results of Mangerel, Ruzsa, Erd\H{o}s
and Hildebrand to Theorem~\ref{thm:main} has been checked in Lean~4.\footnote{\url{https://github.com/FireflySentinel/erdos-1122}}
...
its truncated quadratic prime mass. Ruzsa and Elliott give positive
variance after clipping, whereas Mangerel's short-interval theorem and
the density hypothesis give an incompatible upper bound. Higher prime
powers require no growth restriction.
...
normalizing factor $1/X$. For strongly additive $z(n)=\sum_{p\mid n}a_p$,
...
\]
and $B_j(z,X)^j=\sum_{p^k\le X}|z(p^k)|^j/p^k$ for $j=2,4$.
...
\end{equation}
Indeed, after centering at $\mu_{z_X}(Y)$, each index lies in $[1,Y]$
and occurs at most $H$ times; \eqref{eq:dyadic-center} bounds the center shift.
...
such choices satisfy $c_X\to0$. Put $u_p=f(p)/s_X-c_X\log p$ for
$p\le X$. Then
\begin{align}
...
Since $\clip$ is bounded and Lipschitz, $\E_X|Y-Y_*|^2=o(1)$;
...
Assume that $f$ has no finite concentration and take $Y$ from
\eqref{eq:Y-and-Z}. If $f(n+1)\ge f(n)$, monotonicity and Lipschitz
continuity of $\clip$ give
$(Y(n)-Y(n+1))_+\le |c_X|\log(1+1/n)$, where $t_+=\max\{t,0\}$.
At a decrease of $f$, this quantity is at most $2K$. Since $|Y|\le K$,
...
\eqref{eq:four-terms}, and using \eqref{eq:Y-short}, gives
\begin{align}
...
Erd\H{o}s's Theorem~V now supplies $c$ satisfying
...
$\#\{n\le X:f(n+1)-f(n)<-\varepsilon\}\le D_f(X)=o(X)$, so this law
has no negative mass and must be a point mass at zero. Thus, for every
